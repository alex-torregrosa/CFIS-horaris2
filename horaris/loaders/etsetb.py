...
def loadAssigs(request):
    ...
    for q in cuatris:
        for c in carrs:
            ...
            pars = {"_search": "true", "codPla": c.codigo, "curs": q.codigo[:4], "quad": q.codigo[4]}
            rq = requests.get("https://infoteleco.upc.edu/mason-share/docencia/simul_horaris/dades/assig_oferta.mc", params=pars)
            if not rq.ok:  
                return HttpResponse("ERROR")
            qc = json.loads(rq.text)
            qcdata = qc["invdata"] #?
            for ad in qcdata:
                a = Asignatura(name = ad["nom"], codigo = ad["codi"], codiUPC = ad["codi"], carrera = c, cuatri = q, loaded=False)
                a.save()
...
def cargaAssig(assig):
    ...
    pars = {"cod_asig": assig.codigo, "semestre": assig.cuatri.codigo[-1], "any": assig.cuatri.codigo[:-1], "cod_pla": assig.carrera.codigo, "_search": "true"}
    #visca els indexs negatius de python
    ra = requests.get("https://infoteleco.upc.edu/mason-share/docencia/simul_horaris/dades/assig_dades.mc", params=pars)
    if not ra.ok:
    ...
    if normg and subg:
        for g in grups:
            gn = int(g)
            if gn%10 != 0:
                gr = Grupo(name=str(g), assignatura=assig, subgrupo=True, codigo=str(g), horario = json.dumps(grups[g]+grups[str(gn-(gn%10))]))
                gr.save()
    else:
        for g in grups:
            gr = Grupo(name = str(g), assignatura=assig, subgrupo=False, codigo=str(g), horario=json.dumps(grups[g]))
            gr.save()
